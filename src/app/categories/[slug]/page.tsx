...
export default async function CategoryPage({
  ...
  return (
    <div className="animate-fade-in">
      <Link href="/categories" className="inline-flex items-center gap-1.5 text-sm text-slate-500 hover:text-blue-600 mb-6 transition-colors">
        <ArrowLeft size={14} /> 所有分类
      </Link>
      ...
      {posts.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-slate-200/80">
          <p className="text-slate-400">该分类下暂无文章</p>
        </div>
      ) : (
